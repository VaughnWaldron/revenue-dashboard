...
function splitParts(value: number): { wholeStr: string; cents: string } {
  ...
  return { wholeStr, cents: String(cents).padStart(2, '0') };
}

/** Hero currency display — renders cents smaller and lighter than the whole dollar amount, the way Mercury does. */
export function HeroCurrency({ value, animate = true, durationMs = 900 }: { value: number; animate?: boolean; durationMs?: number }) {
  ...
  return (
    <span>
      {wholeStr}
      <span className="text-[0.5em] font-medium text-ink-muted">.{cents}</span>
    </span>
  );
}
